...
createGalleryItem(id, name, stream) {
    ...
    handleNewPeer(peerId) {
        ...
        // For offers, add polite collision handling
    ...
    handleAnswer(data) {
        ...
        if (!pc) return console.error('PeerConnection missing');
    
        console.log(`Signaling state before setting answer: ${pc.signalingState}`);
    
        // Only set the answer if signaling is stable or have-local-offer
        if (pc.signalingState === "have-local-offer" || pc.signalingState === "stable") {
            pc.setRemoteDescription(new RTCSessionDescription(data.answer))
                .then(() => {
                    console.log(`Remote description set successfully for ${data.senderId}`);
                })
                .catch(error => {
            ...
            console.warn(`Skipped setting answer in state ${pc.signalingState} for ${data.senderId}`);
        }
    }
    ...
    async createOffer(peerId) {
        ...
        if (peerConnection) {
            if (peerConnection.signalingState !== "stable") {
                console.warn(`Cannot create offer for ${peerId}. Signaling state: ${peerConnection.signalingState}`);
                return;
            }
        } else {
        ...
        if (peerConnection.signalingState !== "stable") {
            console.warn(`Cannot create offer for ${peerId}. Current signaling state: ${peerConnection.signalingState}`);
            return;
        }
    
        // Safe to create offer now
        try {
            ...
            peerConnection.onicecandidate = (event) => {
                if (event.candidate) {
                    this.socket.send(JSON.stringify({
                        type: 'candidate',
                        target: peerId,
                        senderId:  this.userId, // This would be replaced with actual user ID
                        candidate: event.candidate
                    }));
                }
            };
