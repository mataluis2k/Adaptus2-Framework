(function() {
     ...
     *                    will be covered by this image until the user taps on it.
    ...
    function initVideoPlayer(options) {
      ...
      var heroImage = options.heroImage; // optional overlay image/GIF URL
      var ctaText = options.ctaText;     // optional CTA HTML
      var redirectUrl = options.redirectUrl; // optional redirect URL when video ends
      ...
      // Function to initialize Video.js and setup our event listeners.
      function setupPlayer() {
        var player = videojs('dynamic_video');
        player.ready(function() {
  
          // If a hero overlay exists, attach a click handler to hide it and play the video.
    ...
    // Expose the initVideoPlayer function globally so it can be called by users.
    window.initVideoPlayer = initVideoPlayer;
  
    // Example usage (uncomment to auto-initialize):
    /*
    initVideoPlayer({
      targetDiv: '#videoContainer',     // The selector for the container to inject the player
      ...
      ctaText: '<h2>Special Offer!</h2><p>Don\'t miss out on this exclusive deal.</p>',
      redirectUrl: '/checkout_page',      // Optional redirect URL when video ends
      offerShowOffset: 45                 // Optional offset (in seconds) before video end to show CTA
    });
    */
  })();
// End of adaptus2-player.js  