...
;
    ...
    async loginUser(ctx, params) {
        ...
        try {
            ...
            if(!authenticated)
            {   
                const result = this.makeFailResponse(ctx);
                this.response.setResponse(400, "Error authenticating user.", "", result, "loginPlugin");
            ...
            const result = this.loginFailResponse(ctx);
            this.response.setResponse(400, "Error authenticating user.", "", result, "loginPlugin");
            return { success: false, result, key: 'response' };
